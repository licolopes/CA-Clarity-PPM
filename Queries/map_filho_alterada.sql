...
FROM 	(
		...
		WHERE  l.lookup_type = 'OBJ_IDEA_PROJECT_CATEGORY' 
		...
		WHERE  l.lookup_type = 'PR_TIPO_ALT_PROD_PROJ' 
			AND l.language_code = @WHERE:PARAM:LANGUAGE@ 
			AND l.is_active = 1
			AND (
					@WHERE:PARAM:USER_DEF:STRING:MAPPING_TYPE_CONSTRAIN@ IS NULL OR
					@WHERE:PARAM:USER_DEF:STRING:MAPPING_TYPE_CONSTRAIN@ = 'tipo_ind_pos_reg'
				)
		) MAP
WHERE  @FILTER@
SELECT 'TOP' CLASS
	,CASE 
		WHEN idl.DAYS_LATE_PCT < 1 THEN 'GREEN'
		WHEN idl.DAYS_LATE_PCT < 11 THEN 'YELLOW'
		WHEN idl.DAYS_LATE_PCT >= 11 THEN 'RED'
		ELSE 'GRAY'
		END STATUS
	,COUNT(INVI.ID) QTD
...
WHERE INVI.IS_ACTIVE = 1
	...
	AND INVI.ID NOT IN (
		...
		FROM (
			...
			JOIN INV_PROJECTS INVPTOP ON INVITOP.ID = INVPTOP.PRID
			JOIN ODF_CA_INV OCINVTOP ON INVITOP.ID = OCINVTOP.ID
			WHERE INVITOP.IS_ACTIVE = 1 AND INVPTOP.IS_TEMPLATE = 0
			ORDER BY SCORE DESC
			) TOP
		WHERE ROWNUM <= 10
		)
...
JOIN INV_PROJECTS INVP ON INVI.ID = INVP.PRID
JOIN cop_inv_days_late_v idl ON idl.investment_id = INVI.id
WHERE INVI.IS_ACTIVE = 1 AND INVP.IS_TEMPLATE = 0
GROUP BY CASE 
		WHEN idl.DAYS_LATE_PCT < 1 THEN 'GREEN'
		WHEN idl.DAYS_LATE_PCT < 11 THEN 'YELLOW'
		WHEN idl.DAYS_LATE_PCT >= 11 THEN 'RED'

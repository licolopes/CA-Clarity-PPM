...
STRAT_REV.NAME BUSINESS_OBJECTIVE
, COUNT(INVI.ID) QTD
...
JOIN CMN_LOOKUPS_V STRAT_REV ON STRAT_REV.LOOKUP_CODE = OCINV.san_pri_strg_relev AND STRAT_REV.LOOKUP_TYPE = 'SAN_LKP_STRG_RELEV' AND STRAT_REV.LANGUAGE_CODE = 'en'
...
FROM
	(
		SELECT
			CASE 
				WHEN COST_TABLE.ID IS NULL THEN BENEFIT_TABLE.ID 
				ELSE COST_TABLE.ID 
				END ID
			, CASE 
				WHEN COST_TABLE.DATA IS NULL THEN BENEFIT_TABLE.DATA 

...
(
	...
	, "Total Realizado" 
	, CASE
		WHEN DATEPART(YEAR,"Data") = DATEPART(YEAR,(SELECT MAX(hm_data_corte) FROM NIKU.ODF_CA_HM_DATA_RELATORIO WHERE hm_aprovado = 1)) - 1 THEN 'Ano Anterior'
		WHEN DATEPART(YEAR,"Data") = DATEPART(YEAR,(SELECT MAX(hm_data_corte) FROM NIKU.ODF_CA_HM_DATA_RELATORIO WHERE hm_aprovado = 1)) THEN 'Ano Atual'
		WHEN DATEPART(YEAR,"Data") = DATEPART(YEAR,(SELECT MAX(hm_data_corte) FROM NIKU.ODF_CA_HM_DATA_RELATORIO WHERE hm_aprovado = 1)) + 1 THEN 'Ano Posterior'
		ELSE 'Não Informado'
	...
	(
		...
		(
			SELECT
				...
				, OCPRJ.hm_tipo_com_exec_ti "Tipo - Comitê Executivo"
				...
				, 	(
						...
						WHERE
							...
							AND (
									($P{rec_internos} = 'sim' AND PW.RESOURCE_CLASS = 'RECINT') 
									OR ($P{rec_internos} = 'nao' AND PW.RESOURCE_CLASS != 'RECINT')
									OR ($P{rec_internos} = 'tudo')
								)
							AND DATEPART(YEAR,PW.ENTRYDATE) = DATEPART(YEAR,(SELECT MAX(hm_data_corte) FROM NIKU.ODF_CA_HM_DATA_RELATORIO WHERE hm_aprovado = 1)) - 1			
					) "Anos Ant Opex"
				,	(
				...
				, 	(
						...
						WHERE
							...
							AND PMPRJ.COST_TYPE = 'OPERATING'
							AND (
									($P{rec_internos} = 'sim' AND RES_CLASS.RESOURCE_CLASS = 'RECINT') 
									OR ($P{rec_internos} = 'nao' AND RES_CLASS.RESOURCE_CLASS != 'RECINT')
									OR ($P{rec_internos} = 'tudo')
								)
							AND DATEPART(YEAR,FP_COST.START_DATE) = DATEPART(YEAR,(SELECT MAX(hm_data_corte) FROM NIKU.ODF_CA_HM_DATA_RELATORIO WHERE hm_aprovado = 1))		
					) "Ano Atual Prev Opex"
		...
		WHERE
			$X{IN,"Diretoria Executiva",dir_exec}
...
WHERE
	"Status" IS NOT NULL
	AND	$X{IN,"Status",status_dashboard_finan}
